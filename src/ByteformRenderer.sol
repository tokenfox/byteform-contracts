...
contract ByteformRenderer {
    ...
    function renderTokenImageURI(address byteContract, address formContract) external view returns (string memory) {
        string memory base64Image = Base64.encode(_generateImage(byteContract, formContract));
    ...
    function renderTokenImage(address byteContract, address formContract) external view returns (string memory) {
        return string(
            abi.encodePacked('<?xml version="1.0" encoding="UTF-8"?>\n', _generateImage(byteContract, formContract))
        );
    }
    ...
    function _generateMetadata(address byteContract, address formContract) internal view returns (string memory) {
    ...
    function _generateImage(address byteContract, address formContract) internal view returns (bytes memory) {
    ...
    function _generateTextOverlay(uint8[256] memory forms) internal pure returns (string memory) {
        DynamicBufferLib.DynamicBuffer memory buffer;

        buffer.p(
            "<style>.m{width:256px;height:256px}",
            '.g{box-sizing:border-box;width:256px;height:256px;font-family:"IBM Plex Mono",serif,monospace;font-weight:400;font-size:16px;line-height:16px;',
            "display:grid;grid-template-columns:repeat(16,16px);grid-template-rows:repeat(16,16px)}",
            ".g p{margin:0;text-align:center;color:#000;}</style>"
        );
    ...
    function _getFontURI() internal view returns (string memory) {
        return string.concat("data:font/woff2;base64,", IFileStore(FILE_STORE).readFile("IBMPlexMono-Regular.woff2"));
    }
    ...
    function _generateLines() internal pure returns (bytes memory) {
        ...
        buffer.p('<g class="lines hidden"><style>.tr{stroke:#e0e0e0;stroke-width:0.5}</style>');
